  ...
  async announce(
    ...
    if (!announcementChannelId) {
      return await interaction.followUp({
        content: 'No announcement channel ID set in `.env`.',
      });
    }
    ...
    if (!guardData.go) {
      try {
        ...
        await interaction.followUp({
          content: "Looks like you don't have permission, boss.",
        });
      }
      return;
    }
    ...
    if (!channel?.isTextBased()) {
      await interaction.followUp({
        content: "Couldn't find the announcement channel man..",
      });
      return;
    }
    ...
    const thanksMessage = await interaction.followUp({
      content: 'Thank you for using Vetrina Bot Services (VBS).',
    });
