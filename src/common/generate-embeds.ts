...
export async function generateEmbed(eventType: BitbucketEventType, data: BitbucketEvent) {
    ...
    switch(eventType) {
        case BitbucketEventType.COMMENT_NEW:
            embed.description = `${_findUserByName((data as CommentWebhook).comment.user.display_name)} ${(data as CommentWebhook).comment.parent ? 'replied to a comment' : 'commented'} on PR #${data.pullrequest.id}`;
        ...
        case BitbucketEventType.PR_APPROVED:
            embed.description = `${_findUserByName(data.actor.display_name)} approved PR #${data.pullrequest.id}`;
        ...
        case BitbucketEventType.PR_CHANGE_REQUEST:
            embed.description = `${_findUserByName(data.actor.display_name)} requested changes on PR #${data.pullrequest.id}`;
        ...
        case BitbucketEventType.PR_DECLINED:
            embed.description = `${_findUserByName(data.actor.display_name)} declined PR #${data.pullrequest.id}`;
            embed.fields!.push({
                name: `Reason`,
        ...
        case BitbucketEventType.PR_MERGED:
            embed.description = `${_findUserByName(data.actor.display_name)} merged PR #${data.pullrequest.id} (${data.pullrequest.source.branch.name} → ${data.pullrequest.destination.branch.name})`;
        ...
        case BitbucketEventType.PR_NEW:
            embed.description = `${_findUserByName(data.actor.display_name)} created a new PR #${data.pullrequest.id}`;
...
function addCommentFields(embed: EmbedData, data: CommentWebhook) {
    ...
    for (const id of ids) {
        message = message?.replaceAll(`@{${id.replace}}`, `<@${id.with?.discordId ?? id.replace}>`);
    }
    embed.fields?.push({
...
function _findUserByName(name: string) {
    const found = users.find(user => user.name.toLowerCase().includes(name.toLowerCase()) || name.toLowerCase().includes(user.name.toLowerCase()));
    if (!found) {
        return name;
    }
